...
import { Home, Bell, Bookmark, Users, ChevronDown, ChevronRight, User as UserIcon } from 'lucide-react';
import { MOCK_GROUPS, CURRENT_USER } from '../constants';
...
export const Sidebar: React.FC<SidebarProps> = ({ onNavigate, activeView, activeEntityId }) => {
    ...
    const isMyProfileActive = activeView === 'profile' && activeEntityId === CURRENT_USER.id;

    return (
        <div className="bg-radiy-card rounded-3xl p-6 border border-radiy-border/50 shadow-xl h-auto">
            ...
            <ul className="space-y-3">
                ...
                <li>
                    <button
                        onClick={() => onNavigate('profile', CURRENT_USER.id)}
                    ...
                    {isGroupsExpanded && (
                        <div className="mt-2 ml-4 space-y-1 border-l-2 border-radiy-border/50 pl-3 animate-in slide-in-from-top-2 duration-200">
                            {MOCK_GROUPS.map(group => (
                                ...
                                >
                                    <img src={group.avatar} alt={group.name} className="w-6 h-6 rounded-lg object-cover" />
            ...
            <footer className="mt-10 text-[10px] text-radiy-muted px-4 opacity-50 border-t border-radiy-border/30 pt-4">
